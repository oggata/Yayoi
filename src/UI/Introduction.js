...
*/
    ...
    update: function() {
        ...
        if (this.game.storage.tutorialNum == 3 && this.messages.length == 0
            && this.game.mapManager.population >= 5) {
            playSE005_Button(this.game.storage);
        ...
        if (this.game.storage.tutorialNum == 5 && this.messages.length == 0
            && this.game.mapManager.maxFood >= 6) {
            playSE005_Button(this.game.storage);
            this.messages = CONFIG.TUTORIAL_MESSAGE_005;
            this.game.mapManager.amount += 50;
            this.game.addEnemyByMapChip("enemy");
        }
        if (this.game.storage.tutorialNum == 6 && this.messages.length == 0
            && this.game.mapManager.maxSafe >= 6) {
            playSE005_Button(this.game.storage);
            this.messages = CONFIG.TUTORIAL_MESSAGE_006;
            this.game.mapManager.amount += 50;
        }
        if (this.game.storage.tutorialNum == 7 && this.messages.length == 0
            && this.game.mapManager.killedEnemyCount >= 1) {
            playSE005_Button(this.game.storage);
            this.messages = CONFIG.TUTORIAL_MESSAGE_007;
            this.game.mapManager.amount += 50;
        }
        ...
        if (this.game.storage.tutorialNum == 7){
            this.game.header.missionLabel.setString(CONFIG.TUTORIAL_TITLE_007);
        }
        if (this.game.storage.tutorialNum == 8){
